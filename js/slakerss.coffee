...
jQuery ->
	...
	class Feeds extends Backbone.Collection
		model: Feed,
		localStorage: new Backbone.LocalStorage("SlackeRss")
	...
	class FeedsView extends Backbone.View
		...
		initialize: ->
			...
			if @feeds.length == 0
				demo1 = new Feed()
				demo1.set "name":'Demo: HackerNews', "rss":'https://news.ycombinator.com/rss', "position":0
				@feeds.add(demo1)
				demo2 = new Feed()
				demo2.set "name":'Demo: GitHub blog', "rss":'https://github.com/blog.atom', "position":1
				@feeds.add(demo2)
			#@feeds.fetch()
